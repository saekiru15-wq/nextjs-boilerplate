...
if(!s.includes("modern-task-modal-state")){
  s=s.replace('export default function Home(){',
    'export default function Home(){\n const modernTaskModalState=useState(false),taskModalOpen=modernTaskModalState[0],setTaskModalOpen=modernTaskModalState[1],modernSubmitModalState=useState<string|null>(null),submitModalKey=modernSubmitModalState[0],setSubmitModalKey=modernSubmitModalState[1]; // modern-task-modal-state');
}
...
s=s.replace(
  ...
  (m,body)=>m.replace('setProgress("")}catch','setProgress("");setSubmitModalKey(null)}catch')
);
...
const taskSubmit = `function TaskSubmitList({tasks,me,files,setFiles,memos,setMemos,titles,setTitles,bodies,setBodies,submit,users}:{tasks:any[];me:string;files:Record<string,File[]>;setFiles:(v:Record<string,File[]>)=>void;memos:Record<string,string>;setMemos:(v:Record<string,string>)=>void;titles:Record<string,string>;setTitles:(v:Record<string,string>)=>void;bodies:Record<string,string>;setBodies:(v:Record<string,string>)=>void;submit:(t:any)=>Promise<void>;users:any[]}){return <section className="card">{tasks.length?tasks.map((t:any)=>{const key=String(t.id),status=t.statusByUser?.[me]||"미제출",isOpen=submitModalKey===key;return <article className="task task-large" key={t.id}><div className="task-main"><div className="section-title"><div><b>{t.title}</b><div className="muted smalltext">과목: {t.subject||"-"} · 출제자: {userLabel(users.find((u:any)=>u.id===t.creatorId))} · 마감: {t.due||"-"}</div></div><span className={`badge ${statusClass(status)}`}>{status}</span></div><div className="task-desc">{t.desc||t.description||"내용 없음"}</div><AttachmentList attachments={t.attachments}/>{t.feedbackByUser?.[me]&&<div className="feedback">피드백: {t.feedbackByUser[me]}</div>}{status!=="완료"&&<button type="button" className="primary" onClick={()=>setSubmitModalKey(key)}>숙제 제출</button>}</div>{isOpen&&<div className="modern-modal-backdrop" onMouseDown={()=>!busy&&setSubmitModalKey(null)}><section className="modern-modal" role="dialog" aria-modal="true" onMouseDown={e=>e.stopPropagation()}><div className="modern-modal-head"><div><h2>숙제 제출</h2><p className="muted">{t.title}</p></div><button type="button" className="modern-close" onClick={()=>!busy&&setSubmitModalKey(null)}>×</button></div><div className="modern-modal-body"><div className="info-row"><span>과목</span><b>{t.subject||"-"}</b></div><div className="info-row"><span>마감일</span><b>{t.due||"-"}</b></div><label>제출 제목<input value={titles[key]||""} onChange={e=>setTitles({...titles,[key]:e.target.value})} placeholder="제출 제목"/></label><label>제출 내용<textarea value={bodies[key]||""} onChange={e=>setBodies({...bodies,[key]:e.target.value})} placeholder="제출 내용"/></label><label>제출 메모(선택)<textarea value={memos[key]||""} onChange={e=>setMemos({...memos,[key]:e.target.value})} placeholder="제출 메모(선택)"/></label><FilePicker files={files[key]||[]} setFiles={f=>setFiles({...files,[key]:f})}/></div><div className="modern-modal-actions"><button type="button" className="outline" disabled={busy} onClick={()=>setSubmitModalKey(null)}>취소</button><button type="button" className="primary" disabled={!titles[key]?.trim()||!bodies[key]?.trim()||busy} onClick={()=>submit(t)}>제출하기</button></div></section></div>}</article>}) : <Empty text="현재 나에게 배정된 숙제가 없습니다."/>}</section>}`;
