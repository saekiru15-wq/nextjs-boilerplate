...
if(!s.includes('dailyProjectWords')){
  s=s.replace(
    ...
    ' const[dailyProjectWords,setDailyProjectWords]=useState<{word:string;meaning:string}[]>([]),[dailyProjectLoading,setDailyProjectLoading]=useState(false);\n const[aiText,setAiText]=useState("")'
  );
}

if(!s.includes('daily-vocab-extra-220.json')){
  s=s.replace(
    ' useEffect(()=>{refresh()},[]);',
    ' useEffect(()=>{refresh()},[]);\n useEffect(()=>{if(active!=="📚 데일리 프로젝트"||dailyProjectWords.length||dailyProjectLoading)return;setDailyProjectLoading(true);Promise.all([fetch("/daily-vocab-400.json",{cache:"no-store"}),fetch("/daily-vocab-extra-220.json",{cache:"no-store"})]).then(async ([a,b])=>{const x=a.ok?await a.json():[];const y=b.ok?await b.json():[];setDailyProjectWords([...(Array.isArray(x)?x:[]),...(Array.isArray(y)?y:[])])}).catch(()=>setDailyProjectWords([])).finally(()=>setDailyProjectLoading(false))},[active,dailyProjectWords.length,dailyProjectLoading]);'
  );
}

if(!s.includes('DailyProject({')){
  s=s.replace(
    '{active==="🤖 AI 학습도우미"&&<section className="card ai">',
    '{active==="📚 데일리 프로젝트"&&<DailyProject words={dailyProjectWords} loading={dailyProjectLoading}/>} {active==="🤖 AI 학습도우미"&&<section className="card ai">'
  );
