...
s=s.replace('async v=>{await rpc(\"app_update_task\"','async v=>{if((v.files||[]).length>MAX_FILES)throw new Error(`수정 후 첨부파일은 최대 ${MAX_FILES}개까지 가능합니다.`);await rpc(\"app_update_task\"');
