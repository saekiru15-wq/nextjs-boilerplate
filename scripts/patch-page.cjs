...
if (s.includes('setMonth={month}')) throw new Error('invalid calendar prop patch');
if (!s.includes('<Calendar month={month} setMonth={setMonth} events={calendar} users={users}/>')) throw new Error('calendar props not patched');
if (!s.includes('function UpcomingTasks')) throw new Error('upcoming task function missing');
if (!s.includes('수행 대상 및 상태')) throw new Error('assignee status section missing');
