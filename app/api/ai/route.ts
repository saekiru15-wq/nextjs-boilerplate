...
export async function POST(request: NextRequest) {
  try {
    ...
    const input = [...previous, { role: "user", content: [{ type: "text", text: message }] }];
    ...
    const response = await fetch("https://generativelanguage.googleapis.com/v1beta/interactions", {
      ...
      body: JSON.stringify({
        ...
        generation_config: { max_output_tokens: 1200, thinking_level: selectedModel === "gemini-3.5-flash-lite" ? "minimal" : "medium" },
        store: false,
      }),
    });
